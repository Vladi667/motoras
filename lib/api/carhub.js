...
// Brand recognition runs longest-first so multi-word brands match before any
// substring of theirs. Entries are case-insensitive.
const KNOWN_BRANDS = [
 // Auto parts
 'Liqui Moly', 'Castrol', 'Bosch', 'Valeo', 'Motul', 'Total', 'Wurth',
 'Denso', 'Febi', 'Mann', 'NGK', 'SKF', 'Shell', 'Mobil 1', 'Mobil',
 'Elf', 'Fuchs', 'Pentosin', 'Mahle', 'Bilstein', 'Sachs', 'Hella',
 // Detailing & car-care
 "Meguiar's", 'Meguiars', 'Sonax', 'Koch-Chemie', 'Koch Chemie',
 'Carguard', 'Auto Finesse', 'Chemical Guys', 'Gyeon', 'Soft99',
 'Turtle Wax', 'Nanolex', 'Angelwax', 'Adam’s Polishes', 'Adams Polishes',
 'CarPro', 'Poorboy’s', 'Poorboys', 'Optimum', 'Riwax', 'Tunap',
 'Wurth Detailing', 'Riccardo',
 // Tools / equipment
 'Flex', 'Festool', 'Scangrip', 'Makita', 'Milwaukee', 'Rupes', 'Mirka',
 'Bigfoot',
 // Batteries
 'Varta', 'Bosch S4', 'Bosch S5', 'Exide', 'Yuasa', 'Banner', 'Rombat',
 // Other
 'GLOBIZ', 'Paloma', 'Huawei', 'Xiaomi',
...
// Reject brand candidates that look like SKUs, sizes, or product fragments.
// A real brand starts with a letter, has at least 3 chars, contains at most
// one digit, and has no SKU-style separators.
function looksLikeBrand(value) {
 if (!value) return false;
 const trimmed = String(value).trim();
 if (trimmed.length < 3 || trimmed.length > 30) return false;
 if (!/^[A-Za-zÀ-ÿ]/.test(trimmed)) return false; // must start with a letter
 if (/^\d+$/.test(trimmed)) return false; // pure number
 const digitCount = (trimmed.match(/\d/g) || []).length;
 if (digitCount > 2) return false; // too many digits = SKU fragment
 if (/[._\\/]/.test(trimmed)) return false; // SKU separators
 if (/^(set|pack|kit|cutie|box|model|tip|size|nr|nr\.|ref)\b/i.test(trimmed)) return false;
 if (/^[a-z]{1,2}$/i.test(trimmed)) return false; // too short to be a brand
 // Size/units (e.g. "5L", "500 ml")
 if (/^[\d.,]+\s*(gr|kg|ml|l|buc|pcs|cm|mm|m|g|oz)\b/i.test(trimmed)) return false;
 return true;
}
...
function extractBrandFromName(name) {
 ...
 // word-boundary match to avoid e.g. "totalcare" matching "Total".
 const escaped = brand.toLowerCase().replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
 const re = new RegExp(`(^|[^a-z0-9])${escaped}([^a-z0-9]|$)`, 'i');
 if (re.test(nameLower)) return brand;
 }
 // Conservative fallback: only accept a candidate that looks like a real
 // brand. If nothing qualifies, return 'General' rather than ship SKU
 // fragments into the brand facet.
 const parts = String(name || '').split(',');
 for (let index = parts.length - 1; index >= 1; index -= 1) {
 const segment = parts[index].trim();
 if (looksLikeBrand(segment)) return segment;
 }
 return 'General';
}

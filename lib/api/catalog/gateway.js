...
// Phase 3 (Task 4): per-supplier lazy reader. Used by /api/<supplier>
// endpoints so a request scoped to one supplier doesn't trigger reads of
// the other three. Each supplier's catalog is cached independently with
// the same TTL as the global catalog so they refresh in lockstep.
...
// Supplier-internal SKUs that must never surface in the public catalog.
// These are non-product line items (gifts, mystery boxes, advance-payment
// placeholders, packaging) that the supplier feed includes but customers
// must not be able to add to cart.
const PUBLIC_SKU_BLOCKLIST = new Set([
  'Mystery Box',
  'AVM',
  '707',
  'BAG002',
...
function isPubliclyVisible(item) {
  ...
  if (PUBLIC_SKU_BLOCKLIST.has(sku)) return false;
  if (PUBLIC_SKU_BLOCKLIST.has(String(item.id || '').trim())) return false;
...
async function readPublicCatalog() {
  ...
  const ruled = applyAdminRules(items, config);
  return ruled.filter(isPubliclyVisible);
}
...
async function supplierQuery(source, params = {}) {
  ...
  // Phase 3 (Task 4): use the per-supplier cache so /api/<supplier> doesn't
  // trigger global-catalog parsing for all four sources. Filter + paginate
  // happen here against the already-classified per-supplier slice.
  const [items, config] = await Promise.all([
    readSupplierCatalog(source),
    readAdminConfig(),
  ]);
  const filtered = filterItems(items, params);
  const ruled = applyAdminRules(filtered, config);
  return { ok: true, source, ...paginateItems(ruled, params) };
}

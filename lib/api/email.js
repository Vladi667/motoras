...
async function sendContactMessage({ name, email, phone, subject, message }) {
 ...
 const html = `<!DOCTYPE html><html lang="ro"><body style="font-family:Arial,sans-serif;background:#f0f2f7;padding:24px">
 <table width="600" align="center" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 18px rgba(0,0,0,.08)">
 <tr><td style="background:#cc1111;color:#fff;padding:20px 28px;font-size:18px;font-weight:800">Mesaj nou prin formularul de contact</td></tr>
 <tr><td style="padding:24px 28px;color:#12151d;font-size:14px;line-height:1.6">
 <p><strong>De la:</strong> ${safe(name)}</p>
 <p><strong>Email:</strong> <a href="mailto:${safe(email)}">${safe(email)}</a></p>
 ${phone ? `<p><strong>Telefon:</strong> ${safe(phone)}</p>` : ''}
 <p><strong>Subiect:</strong> ${safe(subject)}</p>
 <hr style="border:none;border-top:1px solid #e5e7ec;margin:18px 0"/>
 <p style="white-space:pre-wrap">${safe(message)}</p>
 </td></tr>
 </table></body></html>`;
 return transporter.sendMail({
 from: fromAddress(),
 to: process.env.CONTACT_TO_EMAIL || STORE_EMAIL,
 replyTo: email,
...
async function sendPasswordResetEmail({ to, name, resetUrl }) {
 ...
 const html = `<!DOCTYPE html><html lang="ro"><body style="font-family:Arial,sans-serif;background:#f0f2f7;padding:24px">
 <table width="600" align="center" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 18px rgba(0,0,0,.08)">
 <tr><td style="background:#cc1111;color:#fff;padding:20px 28px;font-size:18px;font-weight:800">Resetare parolă</td></tr>
 <tr><td style="padding:28px;color:#12151d;font-size:14px;line-height:1.6">
 <p>Bună ziua${name ? ', ' + safe(name) : ''},</p>
 <p>Am primit o cerere de resetare a parolei contului tău Motoraș. Apasă pe butonul de mai jos pentru a alege o parolă nouă. Linkul expiră în 30 de minute.</p>
 <p style="text-align:center;margin:28px 0"><a href="${safe(resetUrl)}" style="background:#cc1111;color:#fff;padding:14px 28px;border-radius:9px;font-weight:800;text-decoration:none;display:inline-block">Resetează parola</a></p>
 <p style="font-size:12px;color:#6b7280">Dacă nu ai cerut tu această resetare, poți ignora acest mesaj — parola contului tău nu va fi modificată.</p>
 <p style="font-size:11px;color:#9ca3af;word-break:break-all">Sau copiază acest link în browser: ${safe(resetUrl)}</p>
 </td></tr>
 </table></body></html>`;
 return transporter.sendMail({
 from: fromAddress(),
 to,
 subject: 'Resetare parolă Motoraș',
 html,
 });
}

module.exports = { sendConfirmationEmail, sendTrackingEmail, sendSupplierOrderEmail, sendContactMessage, sendPasswordResetEmail };

...
const DATA_FILE = path.join(process.cwd(), 'supplier-feed-globiz.xml');
...
function parseGlobizStock(stocText) {
 const value = normalizeText(stocText).toLowerCase();
 if (value === 'in stoc') return 5;
 if (value === 'stoc limitat') return 2;
 if (value === 'stoc 0') return 0;
 return 0;
}

function extractImages(block) {
 const images = [];
 const primary = extractField(block, 'imagine');
 if (primary) images.push(primary);
 const innerRe = /<imagini>([\s\S]*?)<\/imagini>/i;
 const innerMatch = block.match(innerRe);
 if (innerMatch) {
 const imgRe = /<imagine>([\s\S]*?)<\/imagine>/gi;
 let m;
 while ((m = imgRe.exec(innerMatch[1])) !== null) {
...
function parseGlobizXml(xml) {
 const items = [];
 const articolRe = /<articol>([\s\S]*?)<\/articol>/g;
 let match;

 while ((match = articolRe.exec(xml)) !== null) {
 const block = match[1];
 const cod = extractField(block, 'cod');
 const denumire = extractField(block, 'denumire');
 const descriere = extractField(block, 'descriere');
 const salePrice = toNumber(extractField(block, 'pret'), 0);
 const listPrice = toNumber(extractField(block, 'pret_intreg'), salePrice);
 const imagine = extractField(block, 'imagine');
 const images = extractImages(block);
 const marca = extractField(block, 'marca') || 'General';
 const categorie = extractField(block, 'categorie');
 const calecategorie = extractField(block, 'calecategorie');
 const stocRaw = extractField(block, 'stoc');
 const stock = parseGlobizStock(stocRaw);
 const barcode = extractField(block, 'cod_bare');
 const um = extractField(block, 'um');
 const weight = toNumber(extractField(block, 'weight'), 0);
 const length = toNumber(extractField(block, 'length'), 0);
 const width = toNumber(extractField(block, 'width'), 0);
 const height = toNumber(extractField(block, 'height'), 0);

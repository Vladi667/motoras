...
async function generateInvoicePdf(order) {
 ...
 // ── TVA-EXEMPT NOTICE ─────────────────────────────────────────────
 // Legal: a non-registered TVA payer must state this explicitly on every
 // fiscal document. Remove this block once the company registers and
 // TVA_ENABLED is set to true above.
 if (!TVA_ENABLED) {
 yT += 28;
 doc.fillColor(MID).font('Helvetica-Oblique').fontSize(8)
 .text('Neplătitor de TVA conform art. 310 din Codul Fiscal (Legea 227/2015).',
 totX - 200, yT, { width: lblW + valW + 200, align: 'right' });
 }

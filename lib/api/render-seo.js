...
function injectSeo(html, p) {
  ...
  const cm = CAT_META[p.cat] || CAT_META.piese;
  const breadcrumb = {
    '@context': 'https://schema.org', '@type': 'BreadcrumbList',
    itemListElement: [
      { '@type': 'ListItem', position: 1, name: 'Acasă', item: SITE + '/' },
      { '@type': 'ListItem', position: 2, name: cm.title, item: SITE + '/' + cm.slug },
      { '@type': 'ListItem', position: 3, name: normalizeText(p.name), item: url },
    ],
  };

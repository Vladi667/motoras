...
function mapIntentToOrder(intent) {
 ...
 tracking_number: intent.metadata?.tracking_number || '',
 tracking_courier: intent.metadata?.tracking_courier || '',
 tracking_sent_at: intent.metadata?.tracking_sent_at || '',
 };
}
...
async function findIntentBySupplierToken(token) {
 if (!token) return null;
 const result = await stripeRequest('/payment_intents', {
 method: 'GET',
 query: { limit: 100, 'expand[0]': 'data.latest_charge' },
 });
 return result.data.find(intent =>
 intent.metadata?.motoras_store === '1' &&
 intent.metadata?.supplier_token === token
 ) || null;
}

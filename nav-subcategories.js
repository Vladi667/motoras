(function () {
 ...
 async function getSubcategoryItems(category) {
 const response = await window.MotApi.getSubcategories(category);
 return (response?.ok ? response.items : []).filter((item) => Number(item.count || 0) > 0);
 }
 ...
 const items = await getSubcategoryItems(category);
 if (!items.length) {
 ...
 const items = (await getSubcategoryItems(category)).slice(0, 5);
 if (!items.length) {
 link.dataset.subcatsReady = '1';
 return;
 }

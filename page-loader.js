(function () {
 ...
 const MIN_DURATION = 0; // no mandatory wait: loader only shows as long as the page takes
 const PRE_NAV_DELAY = 0; // don't block navigation for the loader
 const SHOW_THRESHOLD = 140; // only show the loader for slower navigations
 const EXIT_DURATION = 180;
 const FADE_DURATION = 100;
 const PREFETCH_VIEWPORT_LIMIT = 12;
 ...
 if (anchor.target && anchor.target !== '_self') return false;
 if (anchor.hasAttribute('download')) return false;
 if (anchor.dataset && anchor.dataset.noPrefetch === 'true') return false;
 const href = anchor.getAttribute('href') || '';
 if (!href || href.startsWith('#') || href.startsWith('mailto:') || href.startsWith('tel:') || href.startsWith('javascript:')) return false;
 try {
 const url = new URL(anchor.href, location.href);
 ...
 if (prefetchedHrefs.has(absoluteHref)) return;
 ...
 const warmEvent = function warmLink(event) {
 const anchor = event.target.closest('a');
 if (!anchor) return;
 prefetchAnchor(anchor);
 };

 document.addEventListener('mouseover', warmEvent, { passive: true });
 document.addEventListener('focusin', warmEvent, { passive: true });
 document.addEventListener('touchstart', warmEvent, { passive: true });
 ...
 if (seen.size >= PREFETCH_VIEWPORT_LIMIT) return;
 if (!shouldPrefetchLink(anchor)) return;
 ...
 const elapsed = hasPendingNavigation ? nowMs() - pendingStart : 0;

 if (hasPendingNavigation && elapsed >= SHOW_THRESHOLD) {
